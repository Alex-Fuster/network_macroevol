...
pars$competition_coefficient = 0.03
...
df_divrates <- data.frame(time, S, spec, ext, div_rate)
df_plot_divrates <- melt(df_divrates, id.vars = c("time", "S"))
...
plot_divrates_smoothed <- ggplot(data = df_plot_divrates[df_plot_divrates$variable %in% c("spec", "ext"),], aes(x = time, y = value / S, group = variable)) +
...
plot_divrate <- ggplot(data = df_divrates, aes(x = time, y = div_rate)) +
  geom_line(color = "purple") +
  geom_smooth(method = "loess", color = "darkorchid4") +
  theme_classic() +
  ylab("Diversification Rate")

print(plot_divrates)
print(plot_divrates_smoothed)
print(plot_divrate)
...
list_adj_matrix.f <- list()
  ...
  list_adj_matrix.f[[1]] <- get_adjancency_from_simulation.fw_single(list_networks = res$network_list,
                                           nbasals = pars$Sbasal,
                                           pres_mat = res$presence_matrix)
```
